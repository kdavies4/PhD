...
variable = 'testConditions.caRH' # Varied parameter
unit = '%' # Display unit for the varied parameter
...
# Plot
labels = [label_quantity(round(val, 1), unit) for val in vals]
labels[baseline] += " (baseline)"
multiplot(sims, xname='J', ynames1=['w'], label='Polarization',
          title = 'Cell Polarization: Varying Cathode Inlet Humidity\n'
                  + annotate_conditions(conditions, humidity=False)
                  + ', 80%|x$\,$RH',
...
# Load the results.
runs = map(Results, ['090719-I111-30%-RH-Ca.csv',
                     '090719-I111-Baseline6.csv',
                     '090719-I111-70%-RH-Ca.csv'])
