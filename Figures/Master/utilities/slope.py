...
def slope_marker(origin, slope, size_frac=0.1, pad_frac=0.1, ax=None,
    ...
    fmt : string formatting operators for (rise, run)
    """
    if ax is None:
        ax = plt.gca()
    ...
    if rise is not None:
        try:
           ax.text(x_rise, y_rise, fmt[0]%rise, ha=ha, va='center')
        except TypeError:
           ax.text(x_rise, y_rise, fmt[0], ha=ha, va='center')
        try:
           ax.text(x_run, y_run, fmt[1]%run, va=va, ha='center')
        except TypeError:
           ax.text(x_run, y_run, fmt[1], va=va, ha='center')
    else:
        try:
           ax.text(x_rise, y_rise, fmt[0]%slope, ha=ha, va='center')
        except TypeError:
           ax.text(x_rise, y_rise, fmt[0], ha=ha, va='center')
